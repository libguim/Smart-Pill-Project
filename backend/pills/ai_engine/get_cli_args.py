...
def get_cli_args(job='resnet152', run_phase = 'train', aug_level=0, dataclass='0'):
    ...
    json_pill_label_path_sharp_score = os.path.join(DIR_PROJ, json_pill_label_path_sharp_score)
    json_pill_prescription_type = os.path.join(DIR_PROJ, json_pill_prescription_type)
    json_pill_class_list = os.path.join(DIR_PROJ, json_pill_class_list)
    ...
    # Django 명령어(runserver 등)를 무시하고 정의된 인자만 가져오도록 변경합니다.
