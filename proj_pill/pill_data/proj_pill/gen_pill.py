    ...
    def generate_digits_by_index(self,args, index ) :
        if args.gen_type == 'read_only_image' :
            ...
            try:
                ...
                print(f"⚠️ 파일을 찾을 수 없어 건너뜁니다: {path_img}")
                # 빈 이미지 생성 (args.size_image 또는 args.fittosize 확인)
                size = getattr(args, 'size_image', 224) 
                np_image = np.zeros((size, size, 3), dtype=np.uint8)
            
            # --- 중요: return 문은 try-except 블록 "밖"에 있어야 합니다! ---
