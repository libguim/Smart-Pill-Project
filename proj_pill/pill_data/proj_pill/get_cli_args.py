...
def get_cli_args(job='resnet152', run_phase = 'train', aug_level=0, dataclass='0'):
    ...
    # 경로 수정
    dir_solution_home = r'C:\big_dataproject\Smart-Pill-Project'
    #BATCH_SIZE = 2
    #4월 11일 수정
    BATCH_SIZE = 64
    # num_workers = 2
    ...
    # 4월 11일-> 빼도 무관/유지하고 싶을시 or job == 'resnet 50':
    #if job == 'resnet152':
        #BATCH_SIZE = 12
        #4월 11일 수정:
        #BATCH_SIZE = 64
    ...
    #추가사항 -> 4월 11일
        ...
        model_path_in = os.path.join(DIR_PROJ, f'pill_resnet50_{dataclass}_aug{aug_level}.pt')
        model_path = os.path.join(DIR_PROJ, f'pill_resnet50_{dataclass}_aug{aug_level}.pt')
    ...
    # ... 기존 코드들 사이 적당한 위치에 아래 두 줄을 추가하세요 ...

    parser.add_argument('--job', default=job, help='job name (e.g., resnet152)')
    parser.add_argument('--dir_testimage', default='dir_testimage', help='directory for inference images')
